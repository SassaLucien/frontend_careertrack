...
import { searchPeople, markFeatured, getFeaturedProfiles } from '../services/api';
import FeaturedSuggestionModal from './FeaturedSuggestionModal';
...
const ProfessorProfilesScreen = ({ navigation }) => {
  ...
  const fetchPeople = async () => {
    try {
      ...
      if (result.success) {
        const filteredResults = (result.results || []).filter(p => p.type === 'STUDENT' || p.type === 'TEACHER');
